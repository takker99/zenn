...
[ScrapVim-lite-2](https://scrapbox.io/takker/ScrapVim-lite-2)など以前の prototyping では、全てのコードが完成するまでテストはおろかバグ取りすら出来ませんでした。これでは完成するまで致命的なミスに気づけず、かなりの時間を無駄にすることになります。[ScrapVim-lite-3](https://scrapbox.io/takker/ScrapVim-lite-3)では機能ごとになるべく独立させて作っているので、後はテストを簡単にできる環境を作る必要がありそうです
